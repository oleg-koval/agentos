...
if [[ -n "${AGENTOS_OPS_BIN:-}" ]]; then
  exec "$AGENTOS_OPS_BIN" --entrypoint repository "$@"
elif [[ -x /usr/bin/agentos-ops ]]; then
  exec /usr/bin/agentos-ops --entrypoint repository "$@"
elif [[ -f "$entrypoint_dir/core/go.mod" ]] && command -v go >/dev/null 2>&1; then
  cd "$entrypoint_dir/core"
  exec env AGENTOS_REPO_ROOT="$entrypoint_dir" go run ./cmd/agentos-ops --entrypoint repository "$@"
fi
